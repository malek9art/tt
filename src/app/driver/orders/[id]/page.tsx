...
import {
  ...
  Camera, Loader2, Navigation, MessageSquare
} from "lucide-react";
...
const STATUS_AR: Record<string,string> = {
  ...
  out_for_delivery:"في الطريق", delivered:"تم التوصيل", failed:"فشل", returned:"مُرتجَع",
};
...
export default function DriverOrderDetail() {
  ...
  useEffect(() => {
    ...
    sb.from("shipments")
      ...
      .single()
      .then(({ data }) => {
        setShip(data as Shipment);
        setNotes((data as Shipment)?.carrier_notes ?? "");
        setPhotoUrl((data as Shipment)?.proof_photo_url ?? null);
        setLoading(false);
      });
  }, [id, router]);
  ...
  const updateStatus = async () => {
    ...
    const action = NEXT_STATUS[ship.status];
    if (!action) return;
    ...
    if (ship.orders?.id) {
      const orderStatus =
        ...
        : "processing";
      await sb.from("orders").update({ status: orderStatus }).eq("id", ship.orders.id);
    }
  ...
  const reportFailed = async () => {
    ...
    await sb.from("shipments").update({ status:"failed", carrier_notes:notes||"فشل التوصيل" }).eq("id", id);
  ...
  const addr   = ship.orders?.address_snapshot;
  const action = NEXT_STATUS[ship.status];
    ...
    <div className="min-h-screen bg-[var(--bg-page)]" dir="rtl">
      ...
      <header className="sticky top-0 z-10 flex h-14 items-center gap-3 border-b border-[var(--border)] bg-[var(--bg-card)] px-4">
        ...
        <div className="flex-1">
          <p className="font-bold text-[var(--text-1)]" dir="ltr">#{ship.orders?.order_number}</p>
          <p className="text-xs text-[var(--text-muted)]">{STATUS_AR[ship.status] ?? ship.status}</p>
        </div>
        {isDone && (
          <CheckCircle size={20} className={ship.status==="delivered" ? "text-green-500" : "text-red-400"}/>
        )}
      </header>
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-4">
        ...
        {addr && (
          <div className="card-base p-4 space-y-3">
            ...
            <div className="space-y-2 text-sm text-[var(--text-2)]">
              ...
              {addr.landmark && (
                <p className="text-[var(--text-muted)] text-xs">علامة: {addr.landmark}</p>
              )}
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <a href={`https://wa.me/${addr.phone.replace(/\D/g,"")}`} target="_blank" rel="noreferrer"
        ...
        <div className="card-base p-4">
          ...
          <div className="space-y-2">
            {(ship.orders?.order_items ?? []).map((item, i) => (
          ...
          <div className="mt-3 pt-3 border-t border-[var(--border)] flex items-center justify-between">
            ...
            <span className="text-lg font-bold text-brand-700 dark:text-accent-400">
              {new Intl.NumberFormat("ar-YE").format(ship.orders?.total_amount ?? 0)} ﷼
            </span>
          </div>
        </div>
        ...
        {!isDone && (
          <div className="card-base p-4">
            ...
            <textarea rows={2} value={notes}
              ...
              placeholder="أضف أي ملاحظة عن التوصيل..."
            ...
            {photoUrl ? (
              ...
              <label className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[var(--border)] p-8 cursor-pointer hover:border-brand-400 transition-colors">
                ...
                {uploading
                  ...
                  : <><Camera size={28} className="text-brand-700"/><p className="text-sm font-medium text-[var(--text-1)]">التقط صورة الإيصال</p><p className="text-xs text-[var(--text-muted)]">اختياري — لإثبات التوصيل</p></>}
              </label>
            )}
          </div>
        )}
        ...
        {/* حالة الشحنة النهائية */}
        {isDone && (
          <div className={`card-base p-5 text-center ${
            ship.status==="delivered" ? "border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20" : "border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20"
          }`}>
