...
export default function RegisterForm() {
  ...
  const handleSend = async () => {
    ...
    const { error: err } = await supabase.auth.signInWithOtp({
      ...
      options: { data: { full_name: form.full_name }, channel: "sms" },
    });
  ...
  const handleVerify = async () => {
    ...
    const { data, error: err } = await supabase.auth.verifyOtp({
      phone: fmt(form.phone), token: otp, type: "sms",
    });
    if (err) { setError("رمز غير صحيح"); }
    else if (data?.user) {
      await supabase.from("profiles")
        .update({ full_name: form.full_name, phone: fmt(form.phone) })
        .eq("id", data.user.id);
      setStep("done");
          ...
          <div className="card-base p-6">
            {step === "done" && (
              <div className="py-8 text-center">
                ...
                <p className="text-sm text-[var(--text-muted)] mt-1">تم إنشاء حسابك</p>
              </div>
            )}
            {step === "form" && (
