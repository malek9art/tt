...
export default function LoginPage() {
  ...
  const handleSendOTP = async () => {
    ...
    if (err) {
      // في بيئة التطوير — نقبل الـ OTP الوهمي
      if (err.message.includes("SMS") || err.message.includes("not configured")) {
        setStep("otp"); setResend(60);
      } else {
        setError(err.message.includes("rate") ? "انتظر قليلاً قبل إعادة المحاولة" : "تعذّر الإرسال، تحقق من الرقم");
      }
    } else {
      setStep("otp"); setResend(60);
    }
  ...
  const handleVerifyOTP = async () => {
    if (otp.length < 4) { setError("أدخل رمز التحقق كاملاً"); return; }
    setLoading(true); setError("");
    const formatted = formatPhone(phone);
        ...
        <div className="w-full max-w-md">
          ...
          <div className="card-base p-6">
            ...
            {step === "phone" && (
              <div className="space-y-4">
                ...
                <button onClick={handleSendOTP} disabled={loading || !phone}
                  ...
                  {loading ? <><Loader2 size={16} className="animate-spin" /> جارٍ الإرسال...</> : <>إرسال رمز التحقق <ArrowLeft size={16} /></>}
                </button>
            ...
            {step === "otp" && (
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[var(--text-2)]">رمز التحقق</label>
                  <input
                  ...
                  <p className="mt-1.5 text-xs text-[var(--text-muted)]">أُرسل إلى {phone}</p>
                </div>
                ...
                <button onClick={handleVerifyOTP} disabled={loading || otp.length < 4}
                  className="btn-primary w-full justify-center">
                  {loading ? <><Loader2 size={16} className="animate-spin" /> جارٍ التحقق...</> : "تأكيد الرمز"}
                </button>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  {resendIn > 0 ? (
                    <span className="text-[var(--text-muted)]">إعادة الإرسال بعد {resendIn}ث</span>
                  ) : (
                    <button onClick={handleSendOTP}
                      className="text-brand-700 hover:text-brand-900 font-medium transition-colors">
                      إعادة إرسال الرمز
                    </button>
                  )}
                </div>
