...
export default function NewProductPage() {
  ...
  const addTag = () => {
    if (tag.trim() && !form.tags.includes(tag.trim())) {
      setF("tags",[...form.tags,tag.trim()]); setTag("");
    }
  };
        ...
        <div className="md:col-span-2 space-y-5">
          <div className="card-base p-5 space-y-4">
            ...
            <div>
              ...
              <input type="text" placeholder="iPhone 15 Pro..." value={form.name_ar} onChange={e=>setF("name_ar",e.target.value)} className={inputCls} />
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs font-medium text-[var(--text-2)]">الاسم بالإنجليزي</label>
                <input type="text" dir="ltr" value={form.name_en} onChange={e=>setF("name_en",e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-[var(--text-2)]">SKU</label>
                <input type="text" dir="ltr" placeholder="IPH15-BK" value={form.sku} onChange={e=>setF("sku",e.target.value)} className={inputCls} />
              </div>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-[var(--text-2)]">الوصف</label>
              <textarea rows={4} value={form.description} onChange={e=>setF("description",e.target.value)} className={inputCls+" resize-none"} />
            </div>
          </div>
          ...
          <div className="card-base p-5 space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <input type="text" placeholder="سنة واحدة" value={form.warranty} onChange={e=>setF("warranty",e.target.value)} className={inputCls} />
              </div>
            </div>
          </div>
          ...
          <div className="card-base p-5 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={addTag} className="btn-ghost border border-[var(--border)] px-3"><Plus size={14}/></button>
            </div>
            {form.tags.length>0&&(
              <div className="flex flex-wrap gap-1.5">
                {form.tags.map(t=>(
                  <span key={t} className="flex items-center gap-1 rounded-full bg-brand-50 dark:bg-brand-900/30 px-2.5 py-1 text-xs text-brand-700 dark:text-accent-400">
                    {t}<button onClick={()=>setF("tags",form.tags.filter(x=>x!==t))} className="hover:text-red-500"><X size={10}/></button>
                  </span>
                ))}
              </div>
            )}
        ...
        <div className="space-y-5">
          <div className="card-base p-5 space-y-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.is_featured} onChange={e=>setF("is_featured",e.target.checked)}
          ...
          <div className="card-base p-5 space-y-4">
            ...
            <div>
              ...
              <select value={form.brand_id} onChange={e=>setF("brand_id",e.target.value)} className={inputCls}>
          ...
          <button onClick={handleSubmit} disabled={loading} className="btn-primary w-full justify-center text-base">
            {loading ? <><Loader2 size={16} className="animate-spin"/> جارٍ الحفظ...</> : <><Save size={16}/> حفظ المنتج</>}
          </button>
        </div>
      </div>
    </div>
