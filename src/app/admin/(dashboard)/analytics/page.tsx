...
import {
  TrendingUp, ShoppingBag, Users, Package, Calendar
} from "lucide-react";

interface DaySale    { date: string; revenue: number; orders: number; }
interface TopProduct { name: string; qty: number; revenue: number; }
interface Summary    { totalRevenue: number; totalOrders: number; totalProducts: number; newCustomers: number; }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {loading ? (
          ...
          { label:"الإيرادات",       value:formatYER(summary?.totalRevenue??0), icon:TrendingUp, color:"text-brand-700 dark:text-accent-400", bg:"bg-brand-50 dark:bg-brand-900/30" },
          { label:"الطلبات",         value:String(summary?.totalOrders??0),     icon:ShoppingBag, color:"text-blue-600",  bg:"bg-blue-50 dark:bg-blue-900/20" },
          { label:"عملاء جدد",       value:String(summary?.newCustomers??0),    icon:Users,       color:"text-purple-600",bg:"bg-purple-50 dark:bg-purple-900/20" },
          { label:"منتجات نشطة",    value:String(summary?.totalProducts??0),   icon:Package,     color:"text-green-600", bg:"bg-green-50 dark:bg-green-900/20" },
      ...
      <div className="card-base p-5">
        <h2 className="font-semibold text-[var(--text-1)] mb-5">المبيعات اليومية — آخر {days} أيام</h2>
                ...
                <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                  <div className="relative w-full flex flex-col justify-end" style={{height:"160px"}}>
                    <div className="w-full rounded-t-sm transition-all duration-300"
                      style={{
                        height:`${Math.max(h, day.revenue>0?3:0)}%`,
                        background: day.revenue>0 ? "linear-gradient(180deg,#09444C,#0a5561)" : "var(--border)",
                      }}
                      title={`${date}: ${formatYER(day.revenue)} (${day.orders} طلب)`}
                    />
                  </div>
                  <span className="text-[9px] text-[var(--text-muted)] text-center leading-tight">{date}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="card-base p-5">
          ...
          {loading ? <div className="space-y-3">{[1,2,3].map(i=><div key={i} className="skeleton h-10 w-full"/>)}</div>
          : topProducts.length===0 ? <p className="text-sm text-[var(--text-muted)] text-center py-8">لا توجد بيانات</p>
          : (
            <div className="space-y-3">
              {topProducts.map((p,i) => (
                <div key={p.name} className="flex items-center gap-3">
                  <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                    i===0?"bg-accent-500 text-brand-900":i===1?"bg-brand-200 text-brand-800":"bg-[var(--border)] text-[var(--text-muted)]"
                  }`}>{i+1}</span>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs text-[var(--text-muted)] whitespace-nowrap">{p.qty} وحدة</span>
                    </div>
                  </div>
        ...
        <div className="card-base p-5">
          ...
          {loading ? <div className="space-y-2">{[1,2,3].map(i=><div key={i} className="skeleton h-8 w-full"/>)}</div>
          : Object.keys(byStatus).length===0 ? <p className="text-sm text-[var(--text-muted)] text-center py-8">لا توجد بيانات</p>
          : (
                ...
                return (
                  <div key={status}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm text-[var(--text-2)]">{ORDER_STATUS_AR[status]??status}</span>
                      <span className="text-sm font-semibold text-[var(--text-1)]">{count} ({pct}%)</span>
                    </div>
                    <div className="h-1.5 w-full rounded-full bg-[var(--border)]">
                      <div className={`h-1.5 rounded-full ${colors[status]??"bg-gray-400"}`}
                        style={{width:`${pct}%`}}/>
                    </div>
                  </div>
                );
